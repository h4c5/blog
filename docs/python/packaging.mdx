...
Lorsque l'on execute l'instruction `from count_lines import count_lines_file`, python cherche le module successivement
dans le répertoire courant (là où python a été lancé) ; puis dans le répertoire d'installation de python
`/usr/lib/python3.11` ; puis dans le répertoire par défaut pour l'installation des packages :
`/usr/lib/python3.11/site-packages`.

Dès que le module est trouvé, il est exécuté dans l'environnement python et les éléments qui y sont définis sont alors
disponibles.

La liste des répertoires dans lequel python cherche les modules est accessible depuis python dans la variable `path` du
module `sys` :
...
modules python et qui de faciliter leur accès en créant un espace de nommage utilisant une notation "pointée" :
...
`count_package` contenant un fichier `__init__.py` dans les répertoires de `sys.path`, i.e. le répertoire courant et les
répertoires par défaut vus plus haut.

Si le package est trouvé, les modules qui y sont présent sont accessibles via la notation pointée.
...
sera ensuite installé par des tiers.
...
Le workflow de distribution ressemble donc un à ça :
...
minoritaire par rapport à `setup.py`, c'est pourquoi nous aborderons les trois formats de fichier pour comprendre leur
forces et leur faiblesses.
...
Le fait qu'il s'agisse d'un fichier python fait à la fois sa force et sa faiblesse. Puisque c'est du python de nombreux
projets construisent certains champs dynamiquement, la version par exemple. C'est donc un format difficile à parser et
à interfacer avec d'autres outils externes.

Puisqu'il s'agit d'un format propre à `setuptools` les distributions de type sdist ne seront installable que si
setuptools a bien été installé sur l'environnement cible et dans une version compatible, ce qui peut très bien ne pas
être le cas.
...
Une autre mauvaise habitude récurrente était
...
De plus l'utilisation du fichier comme un script : `python setup.py` est **déprécié** comme l'indique la documentation
de setuptools :
...
`pyproject.toml`), vous devez spécifier quels sont les packages que vous souhaitez inclure dans votre distribution.

Par défaut, vous devez même censé détailler tous les sous-packages explicitement :
...
-   Commencer avec `setuptools`, `build` et `twine` en suivant
    [le guide de la documentation setuptools](https://setuptools.pypa.io/en/latest/userguide/quickstart.html).
-   Utiliser le fichier [pyproject.toml](https://setuptools.pypa.io/en/latest/userguide/pyproject_config.html) pour
    configurer votre projet.
-   Adopter le src-layout pour prendre de bonnes habitudes et éviter de tomber dans des pièges classiques.
-   Essayer [Poetry](https://python-poetry.org/).

Si vous le souhaitez, j'ai créé un template cookie-cutter basique pour créer un nouveau package python qui est
disponible sur Github. N'hésitez pas à me dire ce que vous en pensez et à me faire des suggestions.

Tout ce que je vous ai raconté ici peut sembler beaucoup et pourtant je n'ai fait que survoler le sujet. En tout cas je
pense que l'on peut s'estimer heureuses et heureux en 2023 lorsque l'on voit ce qu'écrivait le site Sam & Max en 2018 :
